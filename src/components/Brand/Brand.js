...
export default function CreateBrand() {
  ...
  const deleteBrand=(id)=>{
    ...
    if(isExecuted){
    axios.delete(`https://backendapioill.herokuapp.com/api/oil/brand/${id}`).then( () =>{
      window.location.reload(false);
    } )
  }
  }
  useEffect(async()=>{
    await axios.get(`https://backendapioill.herokuapp.com/api/oil/brand`).then( (allOilsUseg) =>{
  ...
  const creatBrandfun = ()=>{
    axios.post('https://backendapioill.herokuapp.com/api/oil/brand',{BrandAr,BrandEn,BrandImage:Imagenew,BrandDiscr}).then( () => {
      window.location.reload(false);
    })
  }
