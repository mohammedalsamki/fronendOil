...
export default function CreateOilUsge() {
  ...
  const deleteOilUsge=(id)=>{
    ...
    if(isExecuted){
    axios.delete(`https://backendapioill.herokuapp.com/api/oil/oilUseg/${id}`).then( () =>{
      window.location.reload(false);
    } )}
  }

  useEffect(()=>{
    axios.get(`https://backendapioill.herokuapp.com/api/oil/oilUseg`).then( (allOilsUseg) =>{
...
const setID=(id,OilUsageAr,OilUsageEn,Specs)=>{
  ...
  const creatOilusgefun = ()=>{
    axios.post('https://backendapioill.herokuapp.com/api/oil/oilUseg',{OilUsageAr,OilUsageEn,Specs}).then( () => {
      window.location.reload(false);
    })
  }
