...
export default function ShowBrakeData() {
  ...
  const deletebrake=(id)=>{
    ...
    if(isExecuted){
    axios.delete(`https://backendapioill.herokuapp.com/api/brake/brake/${id}`).then( () =>{
      alert('delete done')
  ...
  useEffect(async()=>{
    getOptionsunit();
   await axios.get(`https://backendapioill.herokuapp.com/api/brake/brake/`).then( (allbrakes) =>{
  ...
  const sendDataToAPI = () => {
    axios.put(`https://backendapioill.herokuapp.com/api/brake/brake/${_id}`, {
