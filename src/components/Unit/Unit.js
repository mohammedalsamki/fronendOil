...
export default function CreateUnit() {
  ...
  const deleteUnit=(id)=>{
    ...
    if(isExecuted){
    axios.delete(`https://backendapioill.herokuapp.com/api/oil/unit/${id}`).then( () =>{
      window.location.reload(false);
    } )}
  }
  useEffect(()=>{
    axios.get(`https://backendapioill.herokuapp.com/api/oil/unit`).then( (allOilsUnit) =>{
  ...
  const creatUnitFun = ()=>{
    axios.post('https://backendapioill.herokuapp.com/api/oil/unit',{UnitNameEn,UnitNameAr}).then( () => {
      window.location.reload(false);
    })
  }
